...
class TurtleSquareNode : public rclcpp::Node {
 public:
  TurtleSquareNode() : Node("turtle_square_node") {
    ...
    // "/turtle1/cmd_vel"
    velocity_pub = this->create_publisher<geometry_msgs::msg::Twist>(
        "/turtle1/cmd_vel", 10);
    ...
    position_sub = this->create_subscription<turtlesim::msg::Pose>(
        "/turtle1/pose", 10,
        std::bind(&TurtleSquareNode::pose_callback, this,
                  std::placeholders::_1));
 ...
 private:
  ...
  void pose_callback(const turtlesim::msg::Pose::SharedPtr msg) {
    // PostFix 1: for ROLLING users the message is turtlesim_msgs::msg::Pose
    RCLCPP_INFO(this->get_logger(), "Turtle pose: x=%f, y=%f, theta=%f", msg->x,
                msg->y, msg->theta);
  }
  // Timer
