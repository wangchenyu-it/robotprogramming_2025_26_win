...
// BoolData
bool BoolData::getBool() { return _value; }

int BoolData::getInt() { return static_cast<int>(_value); }

ValueType BoolData::type() { return BOOL; }

// NumberData
double NumberData::getDouble() { return static_cast<double>(_value); }

int NumberData::getInt() { return static_cast<int>(_value); }

uint64_t NumberData::getUnsignedInt() { return static_cast<uint64_t>(_value); }

bool NumberData::getBool() { return _value != 0.0; }

float NumberData::getFloat() { return static_cast<float>(_value); }

ValueType NumberData::type() { return NUMBER; }

// StringData
const string& StringData::getString() { return _value; }

ValueType StringData::type() { return STRING; }

// ArrayData
ValueType ArrayData::type() { return ARRAY; }

ArrayData& ArrayData::getArray() { return *this; }

ArrayData::~ArrayData() {
  for (vector<ValueData*>::iterator v_it = _value.begin(); v_it != _value.end();
       delete *(v_it++));
}

void ArrayData::add(ValueData* value) { _value.push_back(value); }

void ArrayData::add(double value) { _value.push_back(new NumberData(value)); }

void ArrayData::add(float value) { _value.push_back(new NumberData(value)); }

void ArrayData::add(int value) { _value.push_back(new NumberData(value)); }

void ArrayData::add(uint64_t value) { _value.push_back(new NumberData(value)); }

void ArrayData::add(bool value) { _value.push_back(new BoolData(value)); }
...
void ArrayData::set(size_t idx, int value) { set(idx, new NumberData(value)); }

void ArrayData::set(size_t idx, uint64_t value) {
  set(idx, new NumberData(value));
}

void ArrayData::set(size_t idx, bool value) { set(idx, new BoolData(value)); }
...
// ObjectData
ValueType ObjectData::type() { return OBJECT; }

ObjectData& ObjectData::getObject() { return *this; }

ObjectData::~ObjectData() {
  for (map<string, ValueData*>::iterator v_it = _value.begin();
       v_it != _value.end(); delete (*(v_it++)).second);
}
...
void ObjectData::setUnsignedInt(const string& name, uint64_t value,
                                const char* description) {
  setField(name, new NumberData(value), description);
}
...
// PointerData
ValueType PointerData::type() { return POINTER; }

Identifiable* PointerData::getPointer() { return _pointer; }

// PointerReference
ValueType PointerReference::type() { return POINTER_REF; }
...
#define STREAM_OPS(field_type, setter, getter)                                 \
  std::pair<const std::string&, field_type&> field(const std::string& nm,      \
                                                   field_type& val) {          \
    return std::pair<const std::string&, field_type&>(nm, val);                \
  }                                                                            \
  ObjectData& operator<<(ObjectData& o,                                        \
                         std::pair<const std::string&, field_type&> f) {       \
  ...
  std::pair<const std::string&, const field_type&> field(                      \
      const std::string& nm, const field_type& val) {                          \
    return std::pair<const std::string&, const field_type&>(nm, val);          \
  }                                                                            \
  ObjectData& operator<<(ObjectData& o,                                        \
                         std::pair<const std::string&, const field_type&> f) { \
    o.setter(f.first, f.second);                                               \
    return o;                                                                  \
  }
