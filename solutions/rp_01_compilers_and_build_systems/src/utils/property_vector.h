...
class PropertyVector_ : public Property_<std::vector<T, AllocatorType_>> {
 public:
  ...
  PropertyVector_(const std::string& name_, const std::string& description_,
                  PropertyContainerBase* config_, const ValueType& value_,
                  bool* flag = 0)
      : Property_<ValueType>(name_, description_, config_, value_, flag) {}

  PropertyVector_(const std::string& name_, const std::string& description_,
                  PropertyContainerBase* config_, bool* flag = 0)
  ...
  inline void pushBack(const T& v) { this->_value.push_back(v); }
  ...
  inline void setValue(int idx, const typename ValueType::value_type& v) {
    this->_value[static_cast<size_t>(idx)] = v;
    this->touch();
  }
