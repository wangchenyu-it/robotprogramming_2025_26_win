...
static ValueData* processData(ValueData* vdata, IdContext& context,
  ...
  switch (vdata->type()) {
    case OBJECT: {
      ...
      for (vector<string>::const_iterator f_it = fields.begin();
           f_it != fields.end(); f_it++) {
        ValueData* fdata = data->getField(*f_it);
        ValueData* replaceData =
            processData(fdata, context, danglingRefs, declaredIDs);
        if (replaceData) {
    ...
    case ARRAY: {
      ...
      for (vector<ValueData*>::const_iterator v_it = v_array->begin();
           v_it != v_array->end(); v_it++) {
        ValueData* replaceData =
            processData(*v_it, context, danglingRefs, declaredIDs);
        if (replaceData) {
          v_array->set(static_cast<size_t>(v_it - v_array->begin()),
                       replaceData);
        }
      }
      break;
...
static Serializable* createInstance(
    ObjectData* odata, const string& type, IdContext& context,
  ...
  // with the same ID is read, so first check if the ID is already in the
  // context
  ...
  if (instance) {
    ...
    if (instance->className() != type) {
      stringstream msg;
      msg << "Trying to overwrite " << instance->className() << " (ID "
          << idValue->getInt() << ") with " << type;
    ...
    try {
      ...
      if (make_shared) {
        ...
        if (ident_raw) {
          ...
          // std::cerr << "obmap populated: " << ident_raw <<  " " <<
          // instances_shared[ident_raw] << std::endl;
  ...
  for (vector<int>::iterator dp_it = danglingPointers.begin();
       dp_it != danglingPointers.end(); dp_it++) {
    waitingInstances[instance].insert(*dp_it);
    danglingReferences[*dp_it].insert(instance);
  }
  for (vector<int>::iterator id_it = declaredIDs.begin();
       id_it != declaredIDs.end(); id_it++) {
    // Further check, just in case the ID was a fake field
    if (context.getById(*id_it)) {
      map<int, set<Serializable*>>::iterator entry =
          danglingReferences.find(*id_it);
      if (entry != danglingReferences.end()) {
        ...
        for (set<Serializable*>::iterator instance_it = instSet.begin();
             instance_it != instSet.end(); instance_it++) {
...
Serializable* Deserializer::readObject() {
  ...
  return createInstance(odata.get(), type, *this, _waitingInstances,
                        _danglingReferences, _instances_shared, false);
}
...
SerializablePtr Deserializer::readObjectShared() {
  ...
  Serializable* s =
      createInstance(odata.get(), type, *this, _waitingInstances,
                     _danglingReferences, _instances_shared, true);

  Identifiable* i = dynamic_cast<Identifiable*>(s);
  if (i) {
    if (!i->getSharedPtr()) {
      throw std::runtime_error(
          "pointer not managed! be consistent with readObject calls");
    }
...
bool Deserializer::readObjectData(ObjectData*& objdata, string& type) {
  ...
  // objdata = _objectParser->readObject(*_serializationContext->inputStream(),
  // type);
  return true;
}

void Deserializer::setFilePath(const string& fpath) {
  if (!_serializationContext) _serializationContext = new SerializationContext;
  _serializationContext->setInputFilePath(fpath);
}

Deserializer::~Deserializer() {
  if (_serializationContext) delete _serializationContext;
