...
template <typename CellType_, typename AllocatorType_, typename CellTraits_>
Matrix_<CellType_, AllocatorType_, CellTraits_>::Matrix_(std::size_t rows_,
                                                         std::size_t cols_) {
  _cols = 0;
  resize(rows_, cols_);
}
...
template <typename CellType_, typename AllocatorType_, typename CellTraits_>
Matrix_<CellType_, AllocatorType_, CellTraits_>&
Matrix_<CellType_, AllocatorType_, CellTraits_>::operator=(
    const Matrix_& other) {
  assign(other);
  return *this;
}
...
template <typename CellType_, typename AllocatorType_, typename CellTraits_>
void Matrix_<CellType_, AllocatorType_, CellTraits_>::fill(
    const CellType_& value) {
  std::fill(_data.begin(), _data.end(), value);
}

template <typename CellType_, typename AllocatorType_, typename CellTraits_>
void Matrix_<CellType_, AllocatorType_,
             CellTraits_>::updateEightNeighborOffsets() {
  if (rows() < 2 || cols() < 2) {
...
template <typename CellType_, typename AllocatorType_, typename CellTraits_>
void Matrix_<CellType_, AllocatorType_, CellTraits_>::resize(
...
template <typename CellType_, typename AllocatorType_, typename CellTraits_>
void Matrix_<CellType_, AllocatorType_, CellTraits_>::copyRegionTo(
    Matrix_<CellType_, AllocatorType_, CellTraits_>& dest, const size_t r_min,
    const size_t r_max, const size_t c_min, const size_t c_max) const {
  if (!inside(r_min, c_min) || !inside(r_max, c_max))
    throw std::runtime_error(__PRETTY_FUNCTION__);
  dest.resize(r_max - r_min, c_max - c_min);
  for (size_t r = 0; r < dest.rows(); ++r) {
    memcpy(&dest.at(r, 0), &at(r + r_min, c_min),
           dest.cols() * sizeof(CellType));
  }
}
...
template <typename CellType_, typename AllocatorType_, typename CellTraits_>
void Matrix_<CellType_, AllocatorType_, CellTraits_>::assign(
    const Matrix_& other) {
  if (!other._data.size()) {
    clear();
    return;
...
bool Matrix_<CellType_, AllocatorType_, CellTraits_>::getSubPixel(
  ...
  const float dx = interpolation_point_.x() - static_cast<float>(x0);
  const float dy = interpolation_point_.y() - static_cast<float>(y0);
