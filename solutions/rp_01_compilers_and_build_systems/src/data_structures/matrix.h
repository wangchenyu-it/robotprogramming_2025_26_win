...
namespace mini_core {
/**
   Implements a simplistic 2D array class, that supports assign(not a physical
   copy) and resize Order is row major. An array of row pointers is dynamically
   constructed for fast scattered access
...
class Matrix_ {
 public:
  ...
  void copyRegionTo(ThisType& dest, const size_t r_min, const size_t r_max,
  ...
  inline Eigen::Vector2i pos(const CellType* c) const {
    ...
    assert(index >= 0 && index < static_cast<int>(_data.size()) &&
           "outside ptr");
  ...
  inline std::vector<CellType, AllocatorType>& data() { return _data; }
  ...
  inline CellType& operator[](const std::size_t& index) {
    if (index >= _data.size()) {
      throw std::out_of_range(
          "[Matrix::[]] index out of range for underlying vector");
    }
    return _data[index];
  }
  ...
  inline CellType& at(const std::size_t& index) {
    if (index >= _data.size()) {
      throw std::out_of_range(
          "[Matrix::at] index out of range for underlying vector");
    }
    return _data[index];
  }
  ...
  bool getSubPixel(CellType& interpolated_value_,
